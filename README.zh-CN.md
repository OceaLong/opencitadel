...
打开 **http://localhost:8088**，登录后在设置中添加 LLM API Key，即可运行第一个 Agent 任务。
...
| 平台 / 后端 | [文档中心](docs/README.zh-CN.md) · [安全模型](docs/architecture/security-model.zh-CN.md) · [事件系统](docs/architecture/events.zh-CN.md) |
...
```bash
cp .env.example .env
# 编辑 .env：设置 BOOTSTRAP_ADMIN_PASSWORD 与 LLM API Key

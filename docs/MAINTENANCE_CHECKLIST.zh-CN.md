...
- [ ] 新增或变更 UI 路由或审批组件 → `ui/README.md` + `ui/README.zh-CN.md`、`frontend-ui.md`（+ 中文）、根 README 模块表
...
- [ ] KB/Codebase 摄取变更 → `knowledge-base-ingestion.md`（+ 中文）、`codebase-reindex.md`（+ 中文）、教程 02（+ 中文）、`execution-kernel.md`（+ 中文）
...
| 上传限制 | `nginx/nginx.conf`、`ui/src/lib/constants.ts`、Execution Policy `knowledge_base.document.max_bytes` |

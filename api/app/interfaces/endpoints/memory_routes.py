...
async def list_memories(
    ...
    entries = await memory_service.list_entries(scope=scope, session_id=session_id, q=q, tags=tag_list)
...
async def create_memory(
    ...
    created = await memory_service.create_entry(entry)
...
async def update_memory(
    ...
    existing = await memory_service.get_entry(entry_id)
    data = existing.model_dump()
    for k, v in request.model_dump(exclude_unset=True).items():
        if v is not None:
            data[k] = v
    updated = MemoryEntry(**data)
    result = await memory_service.update_entry(entry_id, updated)

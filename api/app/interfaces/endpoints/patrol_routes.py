...
def _finding_allowed_actions(
    ...
    """
    check_result = next(
        (item for item in check_results if item.id == finding.check_result_id), None
    )
...
async def list_packs(
    ...
    return ApiResponse.success(
        PatrolPackListResponse(
            items=[PatrolPackResponse.from_domain(item) for item in packs],
            limit=limit,
            offset=offset,
        )
    )
...
async def create_pack(
    ...
    pack = await service.create_pack(
        ...
        config=(
            load_patrol_template(body.template_id or "kubernetes-baseline-v1", body.config)
            if body.template_id or not body.config
            else PatrolPackConfig.model_validate(body.config)
        ),
    )
...
async def get_pack(
    ...
    return ApiResponse.success(
        PatrolPackResponse.from_domain(await service.get_pack(pack_id, ctx.scope))
    )


@router.get(
    "/patrol-packs/{pack_id}/metrics", response_model=ApiResponse[PatrolPackMetricsResponse]
)
...
async def activate_pack(
    ...
    return ApiResponse.success(
        PatrolPackResponse.from_domain(
            await service.activate_pack(pack_id, ctx.scope, ctx.principal.user_id)
        )
    )


@router.post("/patrol-packs/{pack_id}/pause", response_model=ApiResponse[PatrolPackResponse])
async def pause_pack(
    pack_id: str,
    ctx: WorkspaceContext = Depends(get_workspace_context),
    _write_guard=Depends(require_non_auditor),
    service: PatrolPackService = Depends(get_patrol_pack_service),
    ...
    return ApiResponse.success(
        PatrolPackResponse.from_domain(
            await service.pause_pack(pack_id, ctx.scope, ctx.principal.user_id)
        )
    )
...
async def trigger_pack(
    ...
    return ApiResponse.success(
        PatrolRunResponse.from_domain(
            await service.trigger_pack(
                pack_id, ctx.scope, ctx.principal.user_id, idempotency_key=idempotency_key
            )
        )
    )
...
async def list_runs(
    ...
    return ApiResponse.success(
        PatrolRunListResponse(
            items=[PatrolRunResponse.from_domain(item) for item in runs], limit=limit, offset=offset
        )
    )
    ...
    return ApiResponse.success(
        PatrolRunDetailResponse(
            **PatrolRunResponse.from_domain(run).model_dump(),
            ...
            findings=[
                PatrolFindingResponse.from_domain(
                    item, allowed_actions=_finding_allowed_actions(item, results, run)
                )
...
async def download_evidence(
    run_id: str,
    ctx: WorkspaceContext = Depends(get_workspace_context),
    service: PatrolEvidenceService = Depends(get_patrol_evidence_service),
):
    payload = await service.build_package(run_id, ctx.scope, actor_user_id=ctx.principal.user_id)
    headers = {
        "Content-Disposition": f'attachment; filename="patrol-{run_id}.zip"',
...
async def cancel_run(
    ...
    return ApiResponse.success(
        PatrolRunResponse.from_domain(
            await service.cancel_run(run_id, ctx.scope, ctx.principal.user_id)
        )
    )
...
async def replay_run(
    ...
    return ApiResponse.success(
        PatrolRunResponse.from_domain(
            await service.replay_run(run_id, ctx.scope, ctx.principal.user_id)
        )
    )
...
async def _decide(
    ...
    finding = await service.decide_finding(
        finding_id, ctx.scope, ctx.principal.user_id, status, body.reason
    )
    ...
    return ApiResponse.success(
        PatrolFindingResponse.from_domain(finding, allowed_actions=allowed_actions)
    )


@router.post(
    "/patrol-findings/{finding_id}/acknowledge", response_model=ApiResponse[PatrolFindingResponse]
)
...
@router.post(
    "/patrol-findings/{finding_id}/resolve", response_model=ApiResponse[PatrolFindingResponse]
)
...
@router.post(
    "/patrol-findings/{finding_id}/false-positive",
    response_model=ApiResponse[PatrolFindingResponse],
)
async def false_positive_finding(
    finding_id: str,
    body: FindingDecisionRequest,
    ctx: WorkspaceContext = Depends(get_workspace_context),
    _write_guard=Depends(require_non_auditor),
...
@router.post(
    "/patrol-findings/{finding_id}/remediations",
...
@router.get(
    "/patrol-runs/{run_id}/remediations", response_model=ApiResponse[PatrolRemediationListResponse]
)
async def list_remediations(
    ...
    return ApiResponse.success(
        PatrolRemediationListResponse(
            items=[PatrolRemediationResponse.from_domain(item) for item in items]
        )
    )


@router.get(
    "/patrol-remediations/{remediation_id}", response_model=ApiResponse[PatrolRemediationResponse]
)

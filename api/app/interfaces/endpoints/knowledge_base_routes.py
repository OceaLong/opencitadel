...
async def create_knowledge_base(
    ...
    kb = await service.create_kb(name=request.name, settings=request.settings)
...
async def list_knowledge_bases(
    ...
    items = await service.list_kbs(limit=limit, offset=offset)
...
async def get_knowledge_base(
    ...
    return Response.success(data=_to_kb_response(await service.get_kb(kb_id)))
...
async def ingest_stream(
    ...
    async def generator():
        async for event in service.stream_ingest(kb_id, latest_event_id=event_id):
...
async def reindex(
    ...
    return Response.success(data=_to_kb_response(await service.reindex(kb_id)))
...
async def read_document_in_kb(
    ...
    doc, content = await service.read_document(doc_id, page=page, kb_id=kb_id)
    return Response.success(

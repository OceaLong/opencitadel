...
from app.domain.models.message import MediaAttachment, Message
...
def resolve_capabilities(llm: LLM) -> InferenceCapabilities:
    caps = getattr(llm, "capabilities", None)
    if isinstance(caps, InferenceCapabilities):
        return caps
...
async def build_file_public_url(
    ...
    *,
    ...
    if not file.key:
        return ""
    url = await file_storage.presigned_get_url(
        file.key,
...
async def upload_image_bytes_to_storage(
    ...
    *,
    ...
    url = await build_file_public_url(stored, file_storage)
    if url:
        return url
...
def _attachment_to_memory_part(
    ...
    image_bytes = _compress_image_bytes(
        image_bytes, attachment.mime_type, capabilities.max_image_bytes
    )
...
def build_user_message(
    ...
    capabilities = resolve_capabilities(llm) if llm else InferenceCapabilities()
    ...
    audio_attachments = [
        a
        for a in (vision_attachments or [])
    ...
    for message in messages:
        ...
        for part in content:
            if (
                ...
                and part.get("type") == _IMAGE_REF_TYPE
                and part.get("ref") in ref_set
            ):
                return True
    return False
...
def inflate_messages_for_llm(
    ...
    for message in messages:
        ...
        cleaned["content"] = new_parts or content
        inflated.append(cleaned)
    return inflated
...
def strip_image_parts_from_messages(
    messages: list[dict[str, Any]],
) -> list[dict[str, Any]]:
...
def strip_images_for_tool_call(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for message in messages:
        ...
        had_image = any(
            isinstance(p, dict) and p.get("type") in {_IMAGE_REF_TYPE, "image_url"} for p in content
        )
...
async def prepare_vision_attachments_from_files(
    ...
    for file in files:
        ...
        try:
            ...
            ref_url = (
                await build_file_public_url(stored_file, file_storage) if stored_file.key else ""
            )
            if ref_url:
                attachments.append(
                    MediaAttachment(
...
async def prepare_media_attachments_from_files(
    ...
    for file in files:
        if is_image_mime(file.mime_type):
            attachments.extend(
                await prepare_vision_attachments_from_files([file], llm, file_storage)
            )

...
class GeminiLLM(LLM):
    """Google Gemini LLM实现（stub，待完整实现）"""
    ...
    async def invoke(
        ...
        logger.error("Gemini Provider尚未完整实现，请安装google-genai SDK并完成适配")
        raise ServerRequestsError(
            "Gemini Provider尚未完整实现。请使用OpenAI/Ollama/Azure Provider，或等待后续版本支持。"
        )

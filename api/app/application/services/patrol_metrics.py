...
CHECK_RESULTS = Counter("opencitadel_patrol_check_results_total", "Patrol check results", ("check_id", "status"))
FINDINGS = Counter("opencitadel_patrol_findings_total", "Patrol findings", ("severity", "status"))
EVIDENCE_COMPLETENESS = Gauge("opencitadel_patrol_evidence_completeness_ratio", "Latest Patrol evidence completeness")
COLLECTOR_ERRORS = Counter("opencitadel_patrol_collector_errors_total", "Collector errors", ("code",))


def observe_finalized(run: PatrolRun, results: list[PatrolCheckResult], findings: list[PatrolFinding]) -> None:

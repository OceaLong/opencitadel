...
def test_daily_cron_converts_asia_shanghai_to_utc():
    result = compute_next_run("cron", "0 9 * * *", from_time=datetime(2026, 8, 3, 0, 0, tzinfo=timezone.utc), timezone_name="Asia/Shanghai")
...
def test_interval_ignores_timezone():
    ...
    assert compute_next_run("interval", "60", from_time=now, timezone_name="Moon/Base") == datetime(2026, 8, 3, 0, 1)


def test_nonexistent_dst_time_moves_to_next_valid_occurrence():
    result = compute_next_run("cron", "30 2 * * *", from_time=datetime(2026, 3, 8, 6, 0, tzinfo=timezone.utc), timezone_name="America/New_York")
...
def test_repeated_dst_time_triggers_once():
    ...
    result = compute_next_run("cron", "30 1 * * *", from_time=datetime(2026, 11, 1, 5, 31, tzinfo=timezone.utc), timezone_name="America/New_York")

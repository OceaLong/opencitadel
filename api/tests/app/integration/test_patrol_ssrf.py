...
def test_builtin_pack_exposes_registered_ids_never_raw_url_promql_or_command():
    ...
    assert {check.probe.args["query_id"] for check in config.checks if check.probe.tool == "prom_query"} == {"app-5xx-ratio"}
    assert {check.probe.args["probe_id"] for check in config.checks if check.probe.tool == "http_probe"} == {"primary-endpoint"}

...
class Repo:
    ...
    async def get_pack(self, pack_id, scope=None, for_update=False):
        return (
            self.pack
            if pack_id == self.pack.id and scope and scope.user_id == self.pack.owner_user_id
            else None
        )
    ...
    async def get_run(self, run_id, scope=None, for_update=False):
        return (
            self.run
            if run_id == self.run.id and scope and scope.user_id == self.pack.owner_user_id
            else None
        )

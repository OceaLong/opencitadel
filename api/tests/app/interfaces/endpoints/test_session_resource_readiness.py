...
class _OwnerScopedBindingService:
    def __init__(self) -> None:
        self.binding = SessionResourceBinding(
            id="b1", session_id="s1", resource_kind=ResourceKind.CODEBASE,
            resource_id="cb1", version_id="legacy:cb1", bound_by="u1",
        )
        self.scopes = []
    ...
    async def upgrade(self, session_id, kind, target_version_id, *, actor_id, scope):
        ...
        assert (session_id, kind, target_version_id, actor_id) == (
            "s1", ResourceKind.CODEBASE, "cbv2", "u1",
        )
        return self.binding.model_copy(update={"id": "b2", "version_id": "cbv2", "supersedes_binding_id": "b1"})
...
async def test_binding_list_is_owner_scoped():
    ...
    response = await resource_governance_routes.list_resource_bindings("s1", _ctx(), service)
    assert response.data[0].version_id == "legacy:cb1"

    with pytest.raises(NotFoundError):
        await resource_governance_routes.list_resource_bindings("s1", _ctx("u2"), service)
...
async def test_binding_upgrade_is_owner_scoped_and_reports_old_and_new_ids():
    ...
    service = _OwnerScopedBindingService()
    response = await resource_governance_routes.upgrade_resource_binding(
        "s1", "codebase", UpgradeResourceBindingRequest(target_version_id="cbv2"),
        _ctx(), Principal(user_id="u1"), service,
    )
    ...
    with pytest.raises(NotFoundError):
        await resource_governance_routes.upgrade_resource_binding(
            "s1", "codebase", UpgradeResourceBindingRequest(target_version_id="cbv2"),
            _ctx("u2"), Principal(user_id="u2"), service,
        )

...
function FileCard({
  ...
  return (
    <div
      className={cn(
        "border-border/70 bg-card hover:bg-muted/50 shadow-card flex flex-shrink-0 cursor-pointer items-center gap-3 rounded-xl border p-3 transition-colors",
        role === "user" && "bg-card",
      )}
    ...
    >
      <div className="bg-info/15 text-info flex h-9 w-9 shrink-0 items-center justify-center rounded-md">
        <FileText size={18} />
      </div>
      ...
      <div className="flex max-w-[600px] flex-wrap items-center gap-3">
        ...
        {onViewAllFiles && (
          <Button
            variant="outline"
            size="sm"
            className="border-border/70 bg-card hover:bg-muted/50 text-muted-foreground shadow-card shrink-0 cursor-pointer gap-2 rounded-xl border px-3 py-2"
            style={{ width: CARD_WIDTH, height: CARD_HEIGHT }}

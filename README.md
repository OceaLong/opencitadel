...
| **应用市场** | `/marketplace` | 15 个轻应用（翻译、营养分析、文档转换、运势等） |
...
- **对象存储**：腾讯云 COS（默认）或 MinIO（`STORAGE_PROVIDER=minio`，compose 内置 `manus-minio`）
...
部署分为 **cloud（默认）** 与 **local（全本地）** 两种模式，由 `.env` 顶部两个变量控制：

| 模式 | `COMPOSE_PROFILES` | `STORAGE_PROVIDER` | 对象存储 | LLM |
|------|-------------------|-------------------|---------|-----|
| **cloud**（默认） | 留空 | `cos` | 腾讯云 COS | 外部 API Key（UI 配置） |
| **local** | `local` | `minio` | 内置 MinIO | 宿主机 Ollama |
...
#### 2a. cloud 模式（腾讯云 COS + 外部 LLM）

`.env` 保持默认即可，填写 COS 凭证：
...
启动后在 UI「设置中心 → 模型管理」添加 LLM 并设置默认模型。

#### 2b. local 模式（内置 MinIO + 宿主机 Ollama）
...
POSTGRES_PASSWORD=your_secure_password
NGINX_PORT=8088
# MinIO 默认值开箱可用，无需修改 MINIO_*
```

启动后在 UI 添加本地模型：Provider 选 `ollama`，`base_url=http://host.docker.internal:11434/v1`（需宿主机已运行 Ollama）。

> local 模式下不填 `MINIO_PUBLIC_ENDPOINT` 时，图片会自动回退为 base64 内联，不影响对话功能。
...
# 构建并启动（local 模式会自动启动 manus-minio；cloud 模式不启动）
# 启动顺序: postgres/redis [+ minio] → manus-migrate → api + worker → ui → nginx
...
| `STORAGE_PROVIDER` | ❌ | cos | 对象存储：`cos`（腾讯云）或 `minio`（本地/自建） |
| `COMPOSE_PROFILES` | ❌ | - | docker-compose 模式：`local` 启用内置 MinIO；留空为 cloud |
| `COS_SECRET_ID` | ✅* | - | 腾讯云 COS SecretId（`STORAGE_PROVIDER=cos` 时） |
| `COS_SECRET_KEY` | ✅* | - | 腾讯云 COS SecretKey |
| `COS_BUCKET` | ✅* | - | COS 存储桶名称 |
| `COS_REGION` | ❌ | ap-guangzhou | COS 地域 |
| `MINIO_ENDPOINT` | ✅* | manus-minio:9000 | MinIO 地址（`STORAGE_PROVIDER=minio` 时） |
| `MINIO_ACCESS_KEY` / `MINIO_SECRET_KEY` | ✅* | minioadmin | MinIO 凭证 |
| `MINIO_BUCKET` | ❌ | manus | MinIO 桶名 |
| `MINIO_PUBLIC_ENDPOINT` | ❌ | - | 可选：LLM 抓取图片用的公网/局域网 MinIO 地址 |
...
- **本地模型**：Provider 选 `ollama` 或 `openai`（vLLM 等 OpenAI 兼容服务），`base_url` 填 `http://host.docker.internal:11434/v1`（容器访问宿主机 Ollama）或局域网地址；Ollama 可留空 API Key。
- **本地 Embedding**：在 `api/config.yaml` 配置 `memory.embedding.base_url` / `model`，`EMBEDDING_API_KEY` 可填占位符。

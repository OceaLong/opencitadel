...
### 🏗️ 企业级架构
- **微服务设计**：前后端分离，API 与 Agent Worker 独立部署，支持水平扩展
- **任务解耦**：Redis Streams 消费组 + 独立 Worker 池执行 Agent，API 无状态处理 SSE
- **高可用性**：健康检查、自动重启、服务依赖管理、迁移 init job
- **数据库迁移**：Alembic 独立 migrate job，API 启动时仅校验 schema 版本
- **分布式缓存**：Redis 支持任务 dispatch、事件流与会话状态
...
- **Redis 7**：任务 dispatch 队列、Redis Streams 事件管道、缓存
...
- **实时交互**：SSE 事件流 + Token 增量合并渲染；WebSocket VNC 远程桌面
...
```
              ...
              │ • memory_entries │  │ • task:output  │  │ • Planner/ReAct   │
              │ • embedding      │  │ • cancel ctrl  │  └─────────┬─────────┘
              └──────────────────┘  └────────────────┘            │
                                                        ┌─────────▼────────┐
                                                        │   Sandbox         │
                                                        │  (Internal Net)   │
...
| `MEMORY_VECTOR_ENABLED` | ❌ | false | 启用 pgvector 向量记忆召回 |
| `EMBEDDING_API_KEY` | ❌ | - | Embedding API Key（向量记忆） |
| `EMBEDDING_MODEL` | ❌ | text-embedding-3-small | Embedding 模型名 |
| `OTEL_ENABLED` | ❌ | false | 启用 OpenTelemetry |
| `OTEL_EXPORTER_ENDPOINT` | ❌ | - | OTLP gRPC 导出地址 |
| `VAULT_ADDR` | ❌ | - | HashiCorp Vault 地址（可选） |
| `USE_DB_APP_CONFIG` | ❌ | false | 将 MCP/A2A 配置存入 PostgreSQL |
...
Chart 包含 API/Worker Deployment、HPA、migrate initContainer。

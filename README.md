...
- **任务编排**：自动分解复杂任务为可执行步骤
- **模型管理**：支持多 Provider 模型配置、默认模型切换与会话级模型选择
- **Skill 模板**：内置编程、研究、数据分析、写作等模板，也可自定义系统提示词、工具范围和示例问题
- **上下文管理**：长期记忆、会话内存与任务上下文结合，支持会话级记忆查看、压缩和清理
...
### 💾 数据存储
- **PostgreSQL 16**：关系型数据存储（会话、文件元数据、模型、Skill、长期记忆）
...
```
              ...
              │ • Models/Skills  │  │ • Rate Limit   │  │ • VNC :5901       │
              │ • Memories       │  │                │  │                   │
              └──────────────────┘  └────────────────┘  └───────────────────┘
                        │
              ┌─────────▼────────┐
...
- 「设置中心 → 长期记忆」支持全局记忆与会话记忆。任务开始时会召回相关长期记忆并注入 Agent 上下文。

...
> 生产环境请设置强随机 `API_KEY_SECRET`，并通过 `CORS_ORIGINS` 限制跨域来源；公开接口默认启用基础限流。
...
| 场景 | 推荐配置 |
|------|----------|
| 本地开发 | `SANDBOX_ADDRESS` 留空，API/Worker 通过 `docker.sock` 按会话动态创建沙箱 |
| Docker Compose 生产 | 设置 `SANDBOX_ADDRESS=http://manus-sandbox:8080`，使用固定沙箱容器 |

...
The Actuator drives destructive Kubernetes writes (restart/scale/rollback), so
the MCP endpoint must never be reachable unauthenticated. Mirroring the sandbox
broker (api/app/infrastructure/external/sandbox/broker.py), every HTTP request
must carry ``Authorization: Bearer <token>`` and is compared with
...
MIN_TOKEN_LENGTH = 32
ENV_VAR_NAME = "OPS_ACTUATOR_TOKEN"

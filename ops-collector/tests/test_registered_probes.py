...
async def test_registered_prometheus_query_returns_bounded_numeric_sample(monkeypatch):
    _http_client(monkeypatch, lambda request: httpx.Response(200, json={"status": "success", "data": {"result": [{"metric": {"password": "secret"}, "value": [0, "0.02"]}]}}))
    collector = OpsCollector(CollectorSettings(prometheus_queries={"app-5xx-ratio": PrometheusTarget(base_url="https://prom.example", promql="fixed_query")}))
...
async def test_rate_limit_retries_once_and_returns_exact_code(monkeypatch):
    ...
    collector = OpsCollector(CollectorSettings(prometheus_queries={"q": PrometheusTarget(base_url="https://prom.example", promql="fixed")}))
...
async def test_http_body_prompt_injection_is_not_returned_as_authority(monkeypatch):
    ...
    collector = OpsCollector(CollectorSettings(http_probes={"health": HttpTarget(url="https://service.example/health")}))
...
async def test_backup_age_is_computed_from_registered_metadata(monkeypatch):
    last_success = datetime.now(timezone.utc) - timedelta(hours=30)
    _http_client(monkeypatch, lambda request: httpx.Response(200, json={"state": "success", "last_success_at": last_success.isoformat()}))
    collector = OpsCollector(CollectorSettings(backups={"primary": BackupTarget(status_url="https://backup.example/status")}))
...
async def test_dependency_group_reports_failed_member_as_deterministic_data(monkeypatch):
    ...
    collector = OpsCollector(CollectorSettings(dependencies={"primary": [
...
async def test_pvc_registered_query_is_merged_without_accepting_raw_promql(monkeypatch):
    _http_client(monkeypatch, lambda request: httpx.Response(200, json={"data": {"result": [{"value": [0, "92"]}]}}))
    settings = CollectorSettings(
        allowed_namespaces=["demo"],
        prometheus_queries={"pvc-utilization": PrometheusTarget(base_url="https://prom.example", promql="administrator_owned")},
    )
    result = await OpsCollector(settings, k8s_factory=_K8s).k8s_workload_summary("demo", [], 3600, "pvc-utilization")
    assert result["status"] == "ok"
    assert result["data"]["max_pvc_utilization_percent"] == 92.0
    assert {item["type"] for item in result["evidence"]} == {"summary", "resource_refs", "metric_sample"}

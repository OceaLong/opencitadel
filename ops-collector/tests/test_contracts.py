...
def test_error_enum_is_complete():
    assert {item.value for item in CollectorErrorCode} == {
        "AUTH_FAILED", "TARGET_NOT_FOUND", "NAMESPACE_DENIED", "PROBE_DENIED",
        "UPSTREAM_TIMEOUT", "UPSTREAM_UNAVAILABLE", "OUTPUT_TOO_LARGE",
        "REDACTION_FAILED", "SCHEMA_MISMATCH", "RATE_LIMITED", "INTERNAL_ERROR",
    }

...
class KubernetesReader:
    ...
    def workload_summary(
        self, namespace: str, workload_ids: list[str], window_seconds: int = 3600
    ) -> dict[str, Any]:
        ...
        for kind, items in (
            ...
            for item in items:
                ...
                if allowed and ref not in allowed:
                    continue
                desired = int(
                    getattr(item.status, "replicas", 0)
                    or getattr(item.status, "desired_number_scheduled", 0)
                    or 0
                )
                ready = int(
                    getattr(item.status, "ready_replicas", 0)
                    or getattr(item.status, "number_ready", 0)
                    or 0
                )
                ...
                workloads.append(
                    {"ref": ref, "desired": desired, "ready": ready, "unavailable": missing}
                )
        ...
        return {
            ...
            "pending_pods": sorted(
                (
                    ...
                    for ref, reason in {(item["ref"], item["reason"]) for item in pending_pods}
                ),
                key=lambda item: (item["ref"], item["reason"]),
            ),
            "failed_jobs": sorted(failed_jobs),
            "stale_cronjobs": sorted(stale_cronjobs),
            "node_pressures": sorted(
                node_pressures, key=lambda item: (item["ref"], item["condition"])
            ),
        }
    ...
    def recent_events(self, namespace: str, since_seconds: int, limit: int) -> dict[str, Any]:
        ...
        return {
            "warning_events": rows[:limit],
            "high_risk_count": sum(
                1
                for item in rows
                if item["reason"]
                in {"FailedMount", "FailedScheduling", "FailedAttachVolume", "BackOff"}
            ),
        }

...
class FileReadRequest(BaseModel):
    ...
    start_line: int | None = Field(default=None, description="(可选)读取的起始行, 索引从0开始")
    end_line: int | None = Field(default=None, description="(可选)结束行号, 不包含该行")
    sudo: bool | None = Field(default=False, description="(可选)是否使用sudo权限")
    max_length: int | None = Field(default=10000, description="(可选)要返回的内容的最大长度")
...
class FileWriteRequest(BaseModel):
    ...
    filepath: str = Field(
        ..., description="要写入文件的绝对路径，相对路径将规范化为 /home/ubuntu/ 下"
    )
    content: str = Field(..., description="要写入的文本内容")
    append: bool | None = Field(default=False, description="(可选)是否使用追加模式")
    leading_newline: bool | None = Field(
        default=False, description="(可选)是否在内容开头添加前置空行"
    )
    trailing_newline: bool | None = Field(

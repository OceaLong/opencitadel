...
class ShellExecuteRequest(BaseModel):
    ...
    session_id: str | None = Field(default=None, description="目标 Shell 会话的唯一标识符")
    exec_dir: str | None = Field(default=None, description="执行命令的工作目录(必须使用绝对路径)")
...
class ShellReadRequest(BaseModel):
    ...
    console: bool | None = Field(default=None, description="是否返回控制台记录列表")
...
class ShellWaitRequest(BaseModel):
    ...
    session_id: str = Field(..., description="目标 Shell 会话的唯一标识符")
    seconds: int | None = Field(default=None, description="等待时间, 单位为秒")

...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def app_exception_handler(req: Request, e: AppException) -> JSONResponse:
        """处理 OpenCitadel 沙箱自定义业务异常，将所有状态统一响应结构"""
        logger.error("AppException: %s", e.msg)
        return JSONResponse(
    ...
    async def exception_handler(req: Request, e: Exception) -> JSONResponse:
        """处理 OpenCitadel 沙箱服务中抛出的任意未定义异常，将所有状态码统一响应结构"""
        logger.error("Exception: %s", e)
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=Response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR, msg="服务器出现异常请稍后尝试", data={}
            ).model_dump(),
        )
